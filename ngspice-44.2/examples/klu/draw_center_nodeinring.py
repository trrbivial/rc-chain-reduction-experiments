...
for file_name in file_list:
    ...
    for i, line in enumerate(lines):
        if '.CenterCliqueSummaryNodeInRing' in line:
            # 找到 .END
...
for file_name in file_list:
    ...
    plt.yscale('log')
    plt.xlabel('At Least In x Simple Rings')
    plt.ylabel('Node Count')
    plt.title(meta)
    plt.tight_layout()
    print(meta + "_centernodeinring.png")
    plt.savefig(meta + "_centernodeinring.png", dpi=300)
